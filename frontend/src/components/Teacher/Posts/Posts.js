...
function Posts() {
  ...
  const [open, setOpen] = useState(false);
    ...
    let getUserData = async () => {
      ...
      await axios
        .get(`http://localhost:5000/teacher/posts`, {
          headers: {
            token: token,
          },
        })
  ...
  const setNewPosts = async (e) => {
    if (e.target.value == "All") {
      ...
      setLoading(true);
      await axios
        .get(`http://localhost:5000/teacher/posts/search`, {
          headers: {
            token: token,
          },
          params: {
  ...
  const deletePost = async (id) => {
    await axios
      .delete(`http://localhost:5000/teacher/posts/${id}`, {
        headers: {
          token: token,
        },
      })
          ...
          post.admin ? ( 
            ...
            <li>
              <div className="posts-body">
                ...
                {post.teacher_id === user_id &&
                <div className="posts-footer">
                  <Link to={`/teacher/posts/edit/${post.id}`}  style={{ color: "inherit", textDecoration: "inherit" }}>
                  <Button
                    variant="contained"
                    size="small"
                    startIcon={<EditIcon />}
