...
function AssignmentSubmitions() {
  ...
  let { id, session,subject, discipline } = useParams();
  ...
  useEffect(() => {
    ...
    let getUsersData = async () => {
      await axios
        .get(
          `http://localhost:5000/teacher/assignments/show/assignment-submissions`,
          {
            headers: {
              token: token,
            },
  ...
  const columns = [
    ...
    { field: "cgpa", headerName: "CGPA", width: 100, hide: true },
    {
      ...
      renderCell: (params) => {
        if (params.row.project_oraganizer_assignment_submissions[0]) {
    ...
    },
    {
      ...
      renderCell: (params) => {
        if (params.row.project_oraganizer_assignment_submissions[0]) {
          if (params.row.project_oraganizer_assignment_submissions[0].obtained_marks) {
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <>
            {params.row.project_oraganizer_assignment_submissions[0] && (
              <Link
                to={`/teacher/sessions/students/${session}/${discipline}/assignments/view/${id}/assignment-submissions/assignment/${params.id}`}
