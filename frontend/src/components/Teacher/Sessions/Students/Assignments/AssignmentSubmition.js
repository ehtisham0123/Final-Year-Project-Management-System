...
function AssignmentSubmition() {
  ...
  useEffect(() => {
    let getUserData = async () => {
      ...
      await axios
        .get(
          `http://localhost:5000/teacher/assignments/view-Students-assignnmet/${id}/${student_id}`,
          {
            headers: {
              token: token,
            },
      ...
      let getStudentData = async () => {
        await axios
          .get(`http://localhost:5000/teacher/students/profile/${student_id}`, {
            headers: {
              token: token,
            },
          })
  ...
  const handleSubmit = async (e) => {
    ...
    if (errors.obtained_marks == "") {
      await axios
        .put(
          `http://localhost:5000/teacher/assignments/view-Students-assignnmet/assign-marks`,
          formdata,
          {
            headers: {
              token: token,
