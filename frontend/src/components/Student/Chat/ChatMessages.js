...
function ChatMessages() {
  ...
  useEffect(() => {
    ...
    channel.bind('inserted', function (newMessage) {
      if (user_id == newMessage.student_id && id == newMessage.teacher_id) {
  ...
  useEffect(() => {
    let getTeacherData = async () => {
      await axios
        .get(`http://localhost:5000/student/chat/teacher/${id}`, {
          headers: {
            token: token,
          },
        })
    ...
    let getStudentData = async () => {
      await axios
        .get(`http://localhost:5000/student/chat/avatar`, {
          headers: {
            token: token,
          },
        })
    ...
    let getMessages = async () => {
      await axios
        .get(`http://localhost:5000/student/chat/messages/${id}`, {
          headers: {
            token: token,
          },
        })
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    axios.post('http://localhost:5000/student/chat/messages/new',
      {
        message: input,
        teacher_id: id,
      }, {
      ...
      <div className="col-7 col-md-8 chat">
        <div className="card chat_card">
          <div className="card-body welcome_card_body">
            ...
            <p>Hare You can chat with Teachers </p>
          </div>
        </div>
      </div>
      : <div className="col-7 col-md-8 chat">
