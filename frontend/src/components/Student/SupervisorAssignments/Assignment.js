...
function Assignment() {
  ...
  useEffect(() => {
    let getUserData = async () => {
      setLoading(true);
      await axios
        .get(`http://localhost:5000/student/supervisor-assignments/show/${id}`, {
          headers: {
            token: token,
          },
        })
  ...
  const handleSubmit = async (e) => {
    ...
    fd.append("id", id);
    const url = "http://localhost:5000/student/supervisor-assignments/create";
    axios.post(url, fd, {
      headers: {
        token: token,
  ...
  const deleteAssignment = async (assignmentData) => {
    await axios
      .delete(`http://localhost:5000/student/supervisor-assignments/${assignmentData.id}`, {
        headers: {
          token: token,
        },
      })
                        ...
                        <div className="col-md-12">
                          ...
                          <div className="card bg-transparent py-3 px-2 prof-name mt-4">
                            <div className="d-flex justify-content-between">
                              <h6 className="text-info ml-3">Insert you Assignment file hare</h6>
